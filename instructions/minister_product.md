...
- **agent_id**: `minister_product`
- **inbox**: `queue/inbox/minister_product/`
...
```bash
./scripts/inbox_write.sh minister_XX "質問内容" --from minister_product --type clarification
```

### coordination（同期）
他大臣との進捗同期・完了通知：
```bash
./scripts/inbox_write.sh minister_XX "同期内容" --from minister_product --type coordination
```
...
reusability: 再利用性の根拠
agent_id: minister_product
" --from minister_product --type skill_proposal
```
...
### VISION.md 検証
設計大臣が作成した VISION.md をプロダクト観点で検証:
- ユーザーニーズとの整合性
- ビジネス目標との整合性
- 優先順位の妥当性
