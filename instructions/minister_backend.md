...
```bash
./scripts/inbox_write.sh minister_XX "質問内容" --from minister_be --type clarification
```

### coordination（同期）
他大臣との進捗同期・完了通知：
```bash
./scripts/inbox_write.sh minister_XX "同期内容" --from minister_be --type coordination
```
...
reusability: 再利用性の根拠
agent_id: minister_be
" --from minister_be --type skill_proposal
```
