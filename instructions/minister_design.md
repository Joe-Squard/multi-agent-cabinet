...
- **agent_id**: `minister_design`
- **inbox**: `queue/inbox/minister_design/`
...
```bash
./scripts/inbox_write.sh minister_XX "質問内容" --from minister_design --type clarification
```

### coordination（同期）
他大臣との進捗同期・完了通知：
```bash
./scripts/inbox_write.sh minister_XX "同期内容" --from minister_design --type coordination
```
...
reusability: 再利用性の根拠
agent_id: minister_design
" --from minister_design --type skill_proposal
```
...
### あなたのコレクション
- **Private**: `agent_m_design` — 自分だけが読み書きする長期記憶
- **Shared**: `cabinet_shared` — 全エージェント共有の長期記憶
- **Session file**: `memory/sessions/m_design.md`
